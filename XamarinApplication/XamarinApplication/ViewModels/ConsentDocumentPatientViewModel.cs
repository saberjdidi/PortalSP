...
{
    public class ConsentDocumentPatientViewModel : BaseViewModel
    {
        #region Services
        ...
        private List<ConsentDocument> consentDocumentList;
        ...
        public ObservableCollection<ConsentDocument> ConsentDocument
        {
        ...
        #region Constructors
        public ConsentDocumentPatientViewModel()
        {
           ...
           /* if(Patient.isRepositorySaved == true)
            {
                ...
                consentDocumentList = (List<ConsentDocument>)response.Result;
                ConsentDocument = new ObservableCollection<ConsentDocument>(consentDocumentList);
                IsRefreshing = false;
                ...
                var response = await apiService.GetListWithCoockie<ConsentDocument>(
                 "https://portalesp.smart-path.it",
                 "/Portalesp",
                 "/document/searchConfiguredDocument?sortedBy=code&order=asc",
                 res);
                if (!response.IsSuccess)
                {
                    IsRefreshing = false;
        ...
        #region Sigleton
        static ConsentDocumentPatientViewModel instance;
        public static ConsentDocumentPatientViewModel GetInstance()
        {
            if (instance == null)
            {
                return new ConsentDocumentPatientViewModel();
            }
